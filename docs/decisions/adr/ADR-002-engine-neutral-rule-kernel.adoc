...
adapter (`dq.spark_adapter`) executes the size/completeness subset, and
`dq.portable_config` translates exactly representable HOCON count rules. DataFusion and
non-count-rule compilers, plus differential certification against the legacy PyDeequ
path, remain planned. Adoption of the broader architecture remains proposed until adapter
...
|Shared empty/null/NaN/decimal fixtures and one-scan plan checks; differential PyDeequ testing pending
